...
logger.setLevel(logging.ERROR)  # Set appropriate level
...
@click.command()
@click.option(
    "--template", help="Name of the template you wish to retrieve information for"
)
...
def attach(template, variables):
    ...
    template_variables = {
        ...
        "/512/vpn-instance/ip/route/0.0.0.0/0/next-hop/vpn_512_next_hop_ip_address/address": config.get(
            "vpn_512_next_hop_ip_address"
        ),
        "/512/vpn_512_if_name/interface/if-name": config.get("vpn_512_if_name"),
        "/512/vpn_512_if_name/interface/ip/address": config.get(
            "vpn_512_if_ipv4_address"
        ),
        "/0/vpn-instance/ip/route/0.0.0.0/0/next-hop/mpls_next_hop/address": config.get(
            "mpls_next_hop"
        ),
        "/0/vpn-instance/ip/route/0.0.0.0/0/next-hop/public_internet_next_hop/address": config.get(
            "public_internet_next_hop"
        ),
        "/0/vpn_public_internet_interface/interface/if-name": config.get(
            "vpn_public_internet_interface"
        ),
        "/0/vpn_public_internet_interface/interface/ip/address": config.get(
            "vpn_public_interface_if_ipv4_address"
        ),
        "/0/vpn_mpls_interface/interface/if-name": config.get("vpn_mpls_interface"),
        "/0/vpn_mpls_interface/interface/ip/address": config.get(
    ...
    response = requests.post(
        url=url, data=json.dumps(payload), headers=header, verify=False
    )
    if response.status_code == 200:
...
def detach(target, sysip):
    ...
    payload = {
        "deviceType": "vedge",
        "devices": [{"deviceId": str(target), "deviceIP": str(sysip)}],
    }
